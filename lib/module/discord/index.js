...
class Discord {
  ...
  constructor() {
    ...
    if (config.enabled == 1) {
      resources.discord.client.on('messageReactionAdd', (messageReaction, user) => {
        if (!user.bot && messageReaction.message.guild.id == `${config.server}` && messageReaction.message.channel.id == `${config.channel}`) {
  ...
  generateMessage() {
    ...
    let description = JSON.parse(JSON.stringify(config.embedMessage)).replace(/\\n/g, '\n')
